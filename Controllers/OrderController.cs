...
public async Task<IActionResult> GetUserOrders()
{
        ...
        using (var connection = new MySqlConnection(_connectionString))
        {
            ...
            string query = @"
                SELECT o.*, 
                       ...
                       COALESCE(SUM(oi.TotalPrice), 0) as ItemsTotal
                FROM Orders o
                ...
                using (var reader = await cmd.ExecuteReaderAsync())
                {
                    while (await reader.ReadAsync())
                    {
                        var order = new Dictionary<string, object>
                        {
                            ...
                            ["OrderDate"] = GetDateTimeSafe(reader, "OrderDate"),
                            ["TotalPrice"] = reader.GetInt32("TotalPrice"),
                            ["Status"] = reader.GetString("Status"),
                            ["ServiceFee"] = reader.GetInt32("ServiceFee"),
                            ["ItemsCount"] = reader.GetInt32("ItemsCount"),
                            ["ReservationId"] = GetStringSafe(reader, "ReservationId"),
                            ["Notes"] = GetStringSafe(reader, "Notes"),
                            ["CreatedAt"] = GetDateTimeSafe(reader, "CreatedAt"),
                            ["PaymentMethod"] = GetStringSafe(reader, "PaymentMethod"),
                            ["DeliveryAddress"] = GetStringSafe(reader, "DeliveryAddress"),
                            ["TotalItems"] = reader.GetInt32("TotalItems"),
                            ["ItemsTotal"] = reader.GetInt32("ItemsTotal")
                        };
                        orders.Add(order);
                    }
                }
    ...
    {
        Console.WriteLine($"❌ Hiba a GetUserOrders-ben: {ex.Message}");
        return StatusCode(500, new { success = false, message = "Hiba a rendelések lekérésekor" });
    }
}

// Helper metódusok a biztonságos olvasáshoz
...
private string GetDateTimeSafe(MySqlDataReader reader, string columnName)
{
    try
    {
        var value = reader[columnName];
